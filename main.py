...
def scrap_all_data(url):
    html_text = html_to_html_text(url)
    ...
    if not os.path.isfile(file_name):
        update_data_in_file(parse_cities_names_and_urls(html_text), file_name)
        print('All cities names and URLs written into:', file_name)
    else:
        print('All cities names and URLs already into:', file_name)

    rating_data_all_cities = get_rating_from_url_request(file_name)
    update_data_in_file(rating_data_all_cities, file_name)

    price_data_all_cities = get_rating_from_curl_request(file_name)
    update_data_in_file(price_data_all_cities, file_name)
